...
OUTPUT_NAME=${MODEL_NAME}-SFT


MODEL_PATH=.outputs/${MODEL_NAME}/mm_projector.bin
nohup deepspeed llava/train/train_mem.py \
    ...
    --pretrain_mm_mlp_adapter ${MODEL_PATH} \
    ...
    > logs/${OUTPUT_NAME}.out 2>&1 &

echo logs/${OUTPUT_NAME}.out
