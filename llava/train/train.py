...
from llava.train.modeling_llama import LlamaForCausalLM
import deepspeed
...
class ModelArguments:
    ...
    use_fim: bool = field(default=False)
    loss_method: Optional[str] = field(default="cosine")
...
def preprocess_plain(
    ...
    for source in sources:
        ...
        if DEFAULT_IMAGE_TOKEN in source[0]["value"]:
            ...
            try:
                assert FIM_IMAGE_TOKEN in source[0]["value"]
                source[0]["value"] = FIM_IMAGE_TOKEN
...
def preprocess(
    ...
    if (
        ...
        return preprocess_plain(sources, tokenizer, use_fim)
    if (
        conversation_lib.default_conversation.sep_style
...
class LazySupervisedDataset(Dataset):
    ...
    def modality_lengths(self):
        ...
        for sample in self.list_data_dict:
            ...
            # cur_len = cur_len if "image" in sample and FIM_IMAGE_TOKEN not in sample['conversations'][0]['value'] else -cur_len
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        if "mpt" in model_args.model_name_or_path:
            ...
            model.set_method(model_args.loss_method)
    ...
    if model_args.vision_tower is not None:
        ...
        )  # TODO：加入需要的 token 13b check
        # model.set_tokenizer(tokenizer) 13b 检查
    ...
    rank0_print(data_module["train_dataset"][0]["input_ids"])
    rank0_print(data_module["train_dataset"][0]["labels"])
